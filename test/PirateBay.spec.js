...
import * as Torrent from '../src/PirateBay';
...
describe('Torrent', () => {
  describe('order object to number converter', () => {
    it('should convert orderBy and sortBy with name', () => {
      const searchNumber = Torrent.convertOrderByObject({
        orderBy: 'name',
        sortBy: 'asc'
      });
  ...
  describe('Torrent.getTorrent(id)', function() {
    ...
    it('should have a link', () => {
      chaiExpect(this.torrent).to.have.property(
        'link',
        `${Torrent.baseUrl}/torrent/10676856`
      );
    });
   ...
   * Search
   */
  describe('Torrent.search(title, opts)', function testTorrentSearch() {
    beforeAll(async () => {
      this.searchResults = await Torrent.search('Game of Thrones');
      const [fistSearchResult] = this.searchResults;
      this.fistSearchResult = fistSearchResult;
    });
    ...
    describe('search result', () => {
      ...
      it('should have a link', () => {
        ...
        chaiExpect(this.fistSearchResult.link).to.match(
          new RegExp(`${Torrent.baseUrl}/torrent/\\d+/+`)
        );
      });
  ...
  describe('Torrent.getCategories()', function testGetCategories() {
    beforeAll(async () => {
      this.categories = await torrentCategoryFactory();
      const [subcategory] = this.categories[0].subcategories;
      this.subcategory = subcategory;
    });

    it('should return promise', () => {
      chaiExpect(
        parsePage(`${Torrent.baseUrl}/recent`, parseCategories)
      ).to.be.a('promise');
    });
   ...
   * Get TV show
   */
  describe.only('Torrent.getTvShow(id)', function testGetTvShow() {
    beforeAll(async () => {
